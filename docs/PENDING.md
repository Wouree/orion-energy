...
26 markers across 22 files. The loader strips markers from Markdown bodies, and Nunjucks strips them from
...
|---|---|---|---|
| **P-01** | **Public voice number** | `content/site_settings/general.md` → footer on every page, `/contact/` | The live site carries `+237 6XX XXX XXX` / `+2376XXXXXXXX`. **Every `tel:` link on the site is dead today.** We emptied the field rather than carry the placeholder forward: `site.has_phone` now gates every phone block, so nothing renders. WhatsApp (`237692439292`) is confirmed and wired. One line in the CMS fixes this. |
