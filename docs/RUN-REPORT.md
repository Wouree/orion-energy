...
**One blocks basic function and should be fixed first:**
...
*(P-01, the public voice number, was resolved during the merge — see section 10. It was supplied on
`main` in parallel with this run.)*
...
2. **Watch the first Pages deploy from `main`,** for the reasons in section 6.
...
## 10. Addendum — the merge, 2026-09-09

The branch was merged to `main` on the user's explicit instruction, overriding the standing
"never merge" rule in `CLAUDE.md`, on the reasoning that the live falsehoods outweigh the residual
deployment risk.

**`main` had moved while this run was in flight.** Five commits landed on it, two of them Sveltia CMS
saves — meaning someone was editing the live site through the CMS during the run. They touched exactly
the two files this run restructured, so the merge conflicted, and it was resolved deliberately rather
than mechanically:

| File | Their change | Resolution |
|---|---|---|
| `src/_data/site.json` | Real phone and WhatsApp numbers added (`456ce66`), then two CMS saves | **Their data kept, the file dropped.** The number was ported into `content/site_settings/general.md`. `phone_clean` was normalised from `+237 692 439 292` to `+237692439292`, because that value goes inside `href="tel:"` and should not carry spaces. |
| `src/admin/config.yml` | `base_url` corrected to `auth.wouree.com` (`c6b92cf`), and the five malformed collections fixed (`f93f219`) | **Branch version taken.** They had fixed the same two defects (D-04, D-05) independently on the old six-collection model; this branch's config is a fifteen-collection superset built on the new content model, so nothing of theirs is lost. |

**This resolved P-01.** The public voice line is the same number as WhatsApp. Every footer, `/contact/`,
and the `LocalBusiness` structured data now render a working `tel:` link — they rendered nothing before,
because this run had removed the `+237 6XX XXX XXX` placeholder rather than carry it forward.

**One thing the client should know:** someone was editing through the CMS during this run, and the merge
changes what the CMS edits. Saves now go to `content/`, not `src/_data/site.json`. Any CMS edit made
between commit `25dfc0a` and the merge, and not yet pushed, would target a file that no longer exists.
Worth a word to whoever was in the admin panel.

**Verified after resolving:** clean build, all ten assertions passing over 41 pages, no conflict markers,
no placeholder surviving, and the phone number rendering in all three places it should.
